...
def load_frozen_data(fetcher, path="data/frozen_data.pkl"):
...
class PortfolioBacktester:
    ...
    def backtest_type_1_full_training(self, fetcher=None, sequence_length=30, epochs=30, batch_size=32,
        ...
        """
        Backtesting Type 1: Train and test on full 5-year period
        """
        print("=" * 80)
        print("BACKTESTING TYPE 1: FULL TRAINING PERIOD")
        print("=" * 80)
        if fetcher is None:
            raise ValueError("Fetcher must be passed to backtest_type_1_full_training")
        ...
        if nifty_returns.empty:
            raise ValueError("Unable to fetch NIFTY data for risk aversion estimation")
        ...
        if not nifty_returns.empty:
            ...
            nifty_max_drawdown = float((nifty_cumulative / nifty_cumulative.cummax() - 1).min())
    ...
    def backtest_type_2_out_of_sample(self, fetcher=None, sequence_length=30, epochs=30, batch_size=32,
        ...
        print("=" * 80)
        if fetcher is None:
            raise ValueError("Fetcher must be passed to backtest_type_2_out_of_sample")
